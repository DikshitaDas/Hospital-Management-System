  ...
  protected loadAll(): void {
    ...
    forkJoin({ patients: this.adminApi.getAllPatients(), beds: this.adminApi.getAvailableBeds() }).pipe(
      switchMap(({ patients, beds }) => {
        if (!patients.length) return of({ patients, beds, admitted: [] as { patient: User; admission: Admission }[] });
        return forkJoin(patients.map(p => this.adminApi.getPatientAdmissions(p.id).pipe(
          map(list => list.filter(a => a.status.toUpperCase() === 'ADMITTED').map(a => ({ patient: p, admission: a })))
        ))).pipe(map(n => ({ patients, beds, admitted: n.flat() })));
      })
    ).subscribe({
      next: ({ patients, beds, admitted }) => { this.patients.set(patients); this.beds.set(beds); this.admitted.set(admitted); this.loading.set(false); },
      error: () => { this.errorMsg.set('Failed to load admissions.'); this.loading.set(false); }
    });
  }

  protected submitAdmit(): void {
    this.adminApi.admitPatient(this.admitForm).subscribe({ next: msg => { this.showSnackbar(msg || 'Admitted.'); this.loadAll(); }, error: err => this.showSnackbar(err?.error ?? 'Failed.') });
  }
  protected submitDischarge(): void {
    if (!this.dischargeId) return;
    this.adminApi.dischargePatient(this.dischargeId).subscribe({ next: msg => { this.showSnackbar(msg || 'Discharged.'); this.loadAll(); }, error: err => this.showSnackbar(err?.error ?? 'Failed.') });
  }
  protected submitTransfer(): void {
    this.adminApi.transferPatient(this.transferForm).subscribe({ next: msg => { this.showSnackbar(msg || 'Transferred.'); this.loadAll(); }, error: err => this.showSnackbar(err?.error ?? 'Failed.') });
  }
  protected submitEmergency(): void {
    this.adminApi.emergencyAdmission(this.emergencyForm).subscribe({ next: msg => { this.showSnackbar(msg || 'Emergency admission.'); this.loadAll(); }, error: err => this.showSnackbar(err?.error ?? 'Failed.') });
  }
